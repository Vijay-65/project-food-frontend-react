...
const Header: React.FC = () => {
    ...
    return (
        <>
            <header
                className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled || location.pathname !== '/' ? 'bg-white/90 backdrop-blur-md shadow-md py-3' : 'bg-transparent py-5'
                    }`}
            >
                <div className="max-w-7xl mx-auto px-4 md:px-8 flex items-center justify-between">
                    <Link to="/" className="flex items-center gap-2 relative z-50">
                        <div className="w-10 h-10 bg-primary rounded-xl flex items-center justify-center text-white text-2xl font-bold shadow-lg shadow-primary/30">
                            F
                        </div>
                        <span className={`text-xl font-bold tracking-tight ${isScrolled || location.pathname !== '/' || isMenuOpen ? 'text-text' : 'text-white'}`}>
                    ...
                    <nav className="hidden md:flex items-center gap-8">
                        {navItems.map((item) => (
                            <Link
                                key={item.name}
                                to={item.path}
                                className={`font-medium transition-colors hover:text-primary ${isScrolled || location.pathname !== '/' ? 'text-text' : 'text-white'
                                    }`}
                            >
                                {item.name}
                            </Link>
                    ...
                    <div className="flex items-center gap-2 md:gap-4 relative z-50">
                        <Link
                            to="/cart"
                            className={`relative p-2 rounded-full transition-colors hover:bg-white/10 ${isScrolled || location.pathname !== '/' || isMenuOpen ? 'text-text hover:bg-gray-100' : 'text-white hover:bg-white/20'
                                }`}
                        >
